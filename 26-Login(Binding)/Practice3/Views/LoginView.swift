...
class LoginView: UIView {
    ...
    // 로그인 - 이메일 입력 필드
    private lazy var emailTextField: BindableTextField = {
        ...
        tf.binding { [weak self] text in
            self?.viewModel.emailString.value = text
        }
    ...
    // 로그인 - 비밀번호 입력 필드
    private lazy var passwordTextField: BindableTextField = {
        let tf = BindableTextField()
        tf.frame.size.height = 48
        tf.backgroundColor = UIColor.clear
        tf.textColor = UIColor.white
        tf.tintColor = UIColor.white
        ...
        // 기존에는 viewModel의 set 메서드를 통해 뷰 모델로 데이터를 넘겨주었지만
        // 이제는 아래 바인딩 메서드를 통해 데이터를 뷰 모델로 넘겨 줄 수 있게 됨
        tf.binding { [weak self] text in
            self?.viewModel.passwordString.value = text
        }
        // 원한다면 passwordString에도 콜백 클로저를 설정해 다른 요인에 인해 값이 바뀌면 뷰에 알려줄 수도 있음
        // 그런데, 지금은 뷰에서 데이터가 변경되면(키보드로 값을 입력하면) 뷰 모델에 자동으로 전달해주고 있음 
